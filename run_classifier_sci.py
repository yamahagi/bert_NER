...
class InputExample(object):
  ...
  def __init__(self, guid, text_a, text_b=None, label=None,ner=None):
...
class SciProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "entitied_ner_gold_train.tsv")), "train")
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "entitied_ner_gold_test.tsv")), "test")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label,ner=ner))
        return examples
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["ner"] = create_int_feature(feature.ner)
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      ...
      "ner": tf.FixedLenFeature([seq_length], tf.int64),
  }
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 labels, ner,num_labels, use_one_hot_embeddings):
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, per_example_loss, logits, probabilities) = create_model(
        bert_config, is_training, input_ids, input_mask, segment_ids, label_ids,ner,
